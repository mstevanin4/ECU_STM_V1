...
void readTPS()
{
  ...
  if (configPage1.tpsMin < configPage1.tpsMax)  //ECU_STM2
  {
  ...
  else  // for inverted TPS
  {
    if (currentStatus.tpsADC > configPage1.tpsMin) { tempADC = configPage1.tpsMin; }
    else if(currentStatus.tpsADC < configPage1.tpsMax) { tempADC = configPage1.tpsMax; }
    currentStatus.TPS = map(tempADC, configPage1.tpsMax, configPage1.tpsMin, 100, 0); //Take the raw TPS ADC value and convert it into a TPS% based on the calibrated values
    currentStatus.TPS_time = currentLoopTime; 
   }  //ECU_STM2
}
